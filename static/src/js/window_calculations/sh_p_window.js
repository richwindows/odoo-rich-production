/**
 * SH-P (Sliding + Fixed Height) Window Calculations
 * Contains functions for calculating dimensions for SH-P style windows
 */
odoo.define('rich_production.sh_p_window', [], function (require) {
    'use strict';

    /**
     * Converts imperial measurements to metric (mm)
     * @param {string|number} width - Width in inches
     * @param {string|number} height - Height in inches
     * @param {string|number} fixedHeight - Fixed height in inches
     * @returns {Object} Object with metric measurements
     */
    function convertToMetric(width, height, fixedHeight) {
        // Ensure values are numbers
        const numWidth = parseFloat(width);
        const numHeight = parseFloat(height);
        const numFixedHeight = parseFloat(fixedHeight);
        
        // Convert to mm (1 inch = 25.4 mm)
        const widthMm = numWidth * 25.4;
        const heightMm = numHeight * 25.4;
        const fixedHeightMm = numFixedHeight * 25.4;
        
        return {
            widthInch: numWidth,
            heightInch: numHeight,
            fixedHeightInch: numFixedHeight,
            widthMm,
            heightMm,
            fixedHeightMm
        };
    }

    /**
     * 精确舍入到指定小数位数
     * @param {number} value - 待舍入的值
     * @param {number} decimals - 小数位数
     * @returns {number} 舍入后的值
     */
    function round(value, decimals) {
        const factor = Math.pow(10, decimals);
        return Math.round(value * factor) / factor;
    }
    
    /**
     * 将毫米(mm)转换为英寸(inch)并保留指定小数位
     * @param {number} mm - 毫米值
     * @param {number} decimals - 小数位数，默认为3
     * @returns {number} 转换后的英寸值
     */
    function mmToInch(mm, decimals = 3) {
        return round(mm / 25.4, decimals);
    }

    /**
     * 根据玻璃类型生成玻璃列表数据
     * @param {string} glassType - 玻璃类型
     * @param {number} sashglassw - 滑动玻璃宽度(毫米)
     * @param {number} sashglassh - 滑动玻璃高度(毫米)
     * @param {number} fixedglassw - 固定玻璃宽度(毫米)
     * @param {number} fixedglassh - 固定玻璃高度(毫米)
     * @param {number} fixedglass2w - 固定上部玻璃宽度(毫米)
     * @param {number} fixedglass2h - 固定上部玻璃高度(毫米)
     * @param {boolean} isTempered - 是否顶部强化
     * @returns {Array} 玻璃列表
     */
    function getGlassList(glassType, sashglassw, sashglassh, fixedglassw, fixedglassh, fixedglass2w, fixedglass2h, isTempered) {
        let glassList = [];
        const temperedMark = isTempered ? 'T' : '';
        
        if (glassType === 'Clear/Clear') {
            glassList = [
                {
                    line: 1,
                    qty: 2,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 2,
                    qty: 2,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 2,
                    glassType: 'clear',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add order for tempered glass if needed
            if (isTempered) {
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 2,
                    glassType: 'Clear',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
            }
        } else if (glassType === 'Clear/Low-E270') {
            glassList = [
                {
                    line: 1,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 1,
                    qty: 1,
                    glassType: 'lowe2',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'lowe2',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'lowe2',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add order for tempered glass if needed
            if (isTempered) {
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 1,
                    glassType: 'Clear',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 1,
                    glassType: 'Lowe270',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
            }
        } else if (glassType === 'Clear/Low-E366') {
            glassList = [
                {
                    line: 1,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 1,
                    qty: 1,
                    glassType: 'lowe3',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'lowe3',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'lowe3',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add order for tempered glass if needed
            if (isTempered) {
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 1,
                    glassType: 'Clear',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 1,
                    glassType: 'Lowe366',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
            }
        } else if (glassType === 'OBS/Clear') {
            glassList = [
                {
                    line: 1,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 1,
                    qty: 1,
                    glassType: 'OBS',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'OBS',
                    Tmprd: '',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'OBS',
                    Tmprd: temperedMark,
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add order for tempered glass if needed
            if (isTempered) {
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 1,
                    glassType: 'Clear',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
                glassList.push({
                    line: 'order',
                    lineNumber: 3,
                    qty: 1,
                    glassType: 'P516',
                    Tmprd: 'Tempered',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                });
            }
        } else if (glassType === 'Clear/Clear Tmp') {
            // All glass tempered
            glassList = [
                {
                    line: 1,
                    qty: 2,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 2,
                    qty: 2,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 2,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add orders for all tempered glass
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 2,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 2,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 2,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
        } else if (glassType === 'Clear/Low-E270 Tmp') {
            // All glass tempered
            glassList = [
                {
                    line: 1,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 1,
                    qty: 1,
                    glassType: 'lowe2',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'lowe2',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'lowe2',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add orders for all tempered glass
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 1,
                glassType: 'Lowe270',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 1,
                glassType: 'Lowe270',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 1,
                glassType: 'Lowe270',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
        } else if (glassType === 'Clear/Low-E366 Tmp') {
            // All glass tempered
            glassList = [
                {
                    line: 1,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 1,
                    qty: 1,
                    glassType: 'lowe3',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'lowe3',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'lowe3',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add orders for all tempered glass
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 1,
                glassType: 'Lowe366',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 1,
                glassType: 'Lowe366',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 1,
                glassType: 'Lowe366',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
        } else if (glassType === 'OBS/Clear Tmp') {
            // All glass tempered
            glassList = [
                {
                    line: 1,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },  
                {
                    line: 1,
                    qty: 1,
                    glassType: 'OBS',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(sashglassw).toFixed(2),
                    height: parseFloat(sashglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 2,
                    qty: 1,
                    glassType: 'OBS',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglassw).toFixed(2),
                    height: parseFloat(fixedglassh).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'clear',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                },
                {
                    line: 3,
                    qty: 1,
                    glassType: 'OBS',
                    Tmprd: 'T',
                    Thickness: '3',
                    width: parseFloat(fixedglass2w).toFixed(2),
                    height: parseFloat(fixedglass2h).toFixed(2)
                }
            ];
            
            // Add orders for all tempered glass
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 1,
                qty: 1,
                glassType: 'P516',
                Tmprd: 'Tempered',
                width: parseFloat(sashglassw).toFixed(2),
                height: parseFloat(sashglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 2,
                qty: 1,
                glassType: 'P516',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglassw).toFixed(2),
                height: parseFloat(fixedglassh).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 1,
                glassType: 'Clear',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
            glassList.push({
                line: 'order',
                lineNumber: 3,
                qty: 1,
                glassType: 'P516',
                Tmprd: 'Tempered',
                width: parseFloat(fixedglass2w).toFixed(2),
                height: parseFloat(fixedglass2h).toFixed(2)
            });
        }
        
        // Additional glass types can be added similarly
        
        return glassList;
    }

    /**
     * 处理网格计算
     * @param {Object} windowData - 窗户数据
     * @param {number} sashgridw - 滑动窗网格宽度
     * @param {number} sashgridh - 滑动窗网格高度
     * @param {number} fixedgridw - 固定窗网格宽度
     * @param {number} fixedgridh - 固定窗网格高度
     * @param {number} fixedgrid2w - 固定上部窗网格宽度
     * @param {number} fixedgrid2h - 固定上部窗网格高度
     * @returns {Array} 网格计算结果列表
     */
    function handleGridCalculations(windowData, sashgridw, sashgridh, fixedgridw, fixedgridh, fixedgrid2w, fixedgrid2h) {
        let gridList = [];
        
        console.log('SH-P handleGridCalculations被调用，参数:', {
            windowData: windowData ? {
                id: windowData.id,
                grid: windowData.grid,
                grid_size: windowData.grid_size
            } : 'undefined',
            sashgridw, sashgridh, fixedgridw, fixedgridh, fixedgrid2w, fixedgrid2h
        });
        
        // 确保windowData存在
        if (!windowData) {
            console.warn('windowData为空，无法计算grid');
            return gridList;
        }
        
        // 获取grid值，并转换为小写用于比较
        const gridValue = windowData.grid || '';
        const gridLower = typeof gridValue === 'string' ? gridValue.toLowerCase() : '';
        const quantity = windowData.quantity || 1;
        
        if (gridLower === 'standard') {
            // 从grid_size解析网格尺寸，格式通常为"3w x 3h"
            let gridsquareW = 3; // 默认值
            let gridsquareH = 3; // 默认值
            
            if (windowData && windowData.grid_size) {
                const gridSizeStr = windowData.grid_size;
                console.log('解析grid_size:', gridSizeStr);
                
                // 尝试解析类似"3w x 3h"的格式
                const wMatch = /(\d+)w/i.exec(gridSizeStr);
                const hMatch = /(\d+)h/i.exec(gridSizeStr);
                
                if (wMatch && wMatch[1]) {
                    gridsquareW = parseInt(wMatch[1]);
                    console.log('提取到网格宽度:', gridsquareW);
                }
                
                if (hMatch && hMatch[1]) {
                    gridsquareH = parseInt(hMatch[1]);
                    console.log('提取到网格高度:', gridsquareH);
                }
            }
            
            // SH-P窗户的三个部分网格 (滑动窗, 固定窗, 固定顶部)
            gridList.push({
                sashgridw,
                SashWq: gridsquareH - 1,
                holeW1: sashgridw / gridsquareW,
                sashgridh,
                SashHq: gridsquareW - 1,
                holeH1: sashgridh / gridsquareH,
                fixedgridw,
                FixWq: gridsquareH - 1,
                holeW2: fixedgridw / gridsquareW,
                fixedgridh,
                FixHq: gridsquareW - 1,
                holeH2: fixedgridh / gridsquareH,
                fixedgrid2w,
                FixW2q: gridsquareH - 1,
                holeW3: fixedgrid2w / gridsquareW,
                fixedgrid2h,
                FixH2q: gridsquareW - 1,
                holeH3: fixedgrid2h / gridsquareH
            });
            
        } else if (gridLower === 'marginal') {
            // 边缘型网格
            gridList.push({
                sashgridw,
                SashWq: quantity * 2,
                holeW1: 69.5,
                sashgridh,
                SashHq: quantity * 2,
                holeH1: 102,
                fixedgridw,
                FixWq: quantity * 2,
                holeW2: 102,
                fixedgridh,
                FixHq: quantity * 2,
                holeH2: 102,
                fixedgrid2w,
                FixW2q: quantity * 2,
                holeW3: 102,
                fixedgrid2h,
                FixH2q: quantity * 2,
                holeH3: 102
            });
            
        } else if (gridLower === 'perimeter') {
            // 周边型网格
            gridList.push({
                sashgridw,
                SashWq: quantity * 1,
                holeW1: 69.5,
                sashgridh,
                SashHq: quantity * 1,
                holeH1: 102,
                fixedgridw,
                FixWq: quantity * 1,
                holeW2: 102,
                fixedgridh,
                FixHq: quantity * 1,
                holeH2: 102,
                fixedgrid2w,
                FixW2q: 1,
                holeW3: 102,
                fixedgrid2h,
                FixH2q: 2,
                holeH3: 102
            });
        } else {
            console.warn('不支持的grid类型:', gridValue);
        }
        
        console.log('SH-P handleGridCalculations返回结果:', gridList);
        return gridList;
    }

    /**
     * 计算SH-P窗户的Nailon型材框架尺寸
     * @param {number} widthMm - 窗户宽度(毫米)
     * @param {number} heightMm - 窗户高度(毫米)
     * @param {number} fixedHeightMm - 固定高度(毫米)
     * @param {string} glassType - 玻璃类型
     * @param {Object} windowData - 窗户完整数据
     * @returns {Object} 框架尺寸和其他计算结果
     */
    function calculateSHPNailon(widthMm, heightMm, fixedHeightMm, glassType, windowData) {
        console.log('计算SH-P Nailon窗户尺寸:', { widthMm, heightMm, fixedHeightMm, glassType });
        
        // 框架尺寸计算
        const frameWidth = mmToInch(widthMm + 3 * 2);
        const frameHeight = mmToInch(heightMm + 3 * 2);
        
        // 嵌扇计算
        const sashWidth = mmToInch(widthMm - 47.4 - 15 * 2 - 2);
        const sashHeight = mmToInch((heightMm - fixedHeightMm) / 2 - 6 - 17.1 + 1);
        
        // 屏幕尺寸计算
        const screenWidth = Math.round(widthMm - 87 - 15 * 2 - 4);
        const screenHeight = Math.round((heightMm - fixedHeightMm) / 2 - 6 - 75 - 5);
        
        // 硬件部件计算
        const mullion = mmToInch(widthMm - 36 - 15 * 2);
        const mullionA = round((widthMm - 36 + 1 - 15 * 2) / 25.4 - 2, 1);
        const handleA = Math.round((widthMm - 46 - 15 * 2) / 25.4 / 2 + 4);
        const coverWidth = mmToInch(widthMm - 14 * 2 - 15 * 2 - 3 - 13);
        const coverHeight = mmToInch(fixedHeightMm - 6 - 14 * 2 - 15 - 22 * 2);
        const bigMullion = mmToInch(widthMm - 14 * 2 - 15 * 2 - 2 + 1.5);
        
        // 玻璃尺寸计算
        const sashGlassWidth = widthMm - 110 - 15 * 2 - 2;
        const sashGlassHeight = (heightMm - fixedHeightMm) / 2 - 6 - 79.7 - 1;
        const fixedGlassWidth = widthMm - 47 - 15 * 2;
        const fixedGlassHeight = (heightMm - fixedHeightMm - 6) / 2 - 44.2 - 15 - 1;
        const fixedGlass2Width = widthMm - 47 - 15 * 2;
        const fixedGlass2Height = fixedHeightMm - 6 - 20.5 * 2 - 3 * 2 - 15 - 1;
        
        // 格子尺寸计算
        const sashGridWidth = Math.round(sashGlassWidth - 18 - 2);
        const sashGridHeight = Math.round(sashGlassHeight - 18 - 2);
        const fixedGridWidth = Math.round(fixedGlassWidth - 18 - 2);
        const fixedGridHeight = Math.round(fixedGlassHeight - 18 - 2);
        const fixedGrid2Width = Math.round(fixedGlass2Width - 18 - 2);
        const fixedGrid2Height = Math.round(fixedGlass2Height - 18 - 2);
        
        // 框架清单 - Nailon型材
        const frameList = [
            {
                material: '82-10',
                position: '--',
                length: frameWidth,
                qty: 2
            },
            {
                material: '82-10',
                position: '|',
                length: frameHeight,
                qty: 2
            }
        ];
        
        // 嵌扇清单
        const sashList = [
            {
                material: '82-05',
                position: '--',
                length: sashWidth,
                qty: 1
            },
            {
                material: '82-05',
                position: '--',
                length: sashWidth,
                qty: 1
            },
            {
                material: '82-03',
                position: '|',
                length: sashHeight,
                qty: 2
            }
        ];
        
        // 屏幕清单
        const screenList = [
            {
                material: 'screenw',
                position: '--',
                length: screenWidth,
                qty: 2
            },
            {
                material: 'screenh',
                position: '|',
                length: screenHeight,
                qty: 2
            }
        ];
        
        // 部件清单
        const partsList = [
            {
                material: 'mullion',
                position: '--',
                length: mullion,
                qty: 1
            },
            {
                material: 'mullion aluminum',
                position: '--',
                length: mullionA,
                qty: 1
            },
            {
                material: 'handle aluminum',
                position: '--',
                length: handleA,
                qty: 1
            },
            {
                material: 'cover width',
                position: '--',
                length: coverWidth,
                qty: 1
            },
            {
                material: 'cover height',
                position: '|',
                length: coverHeight,
                qty: 1
            },
            {
                material: 'big mullion',
                position: '--',
                length: bigMullion,
                qty: 1
            }
        ];
        
        // 计算格子清单
        const gridList = handleGridCalculations(
            windowData, 
            sashGridWidth, sashGridHeight, 
            fixedGridWidth, fixedGridHeight,
            fixedGrid2Width, fixedGrid2Height
        );
        
        // 计算玻璃清单
        const glassList = getGlassList(
            glassType, 
            sashGlassWidth, sashGlassHeight,
            fixedGlassWidth, fixedGlassHeight,
            fixedGlass2Width, fixedGlass2Height,
            windowData.top_buttom === 'Tempered'
        );
        
        // 构建结果对象
        const result = {
            frameWidth,
            frameHeight,
            frame: frameList,
            sash: sashList,
            screen: screenList,
            parts: partsList,
            glassList: glassList,
            gridList: gridList
        };
        
        console.log('SH-P Nailon窗户计算结果:', result);
        return result;
    }

    /**
     * 计算SH-P窗户的标准框架尺寸 (Retrofit/Block/Block-slope)
     * @param {number} widthMm - 窗户宽度(毫米)
     * @param {number} heightMm - 窗户高度(毫米)
     * @param {number} fixedHeightMm - 固定高度(毫米)
     * @param {string} frameType - 框架类型 (Retrofit/Block/Block-slope/Block-slope 1/2)
     * @param {string} glassType - 玻璃类型
     * @param {Object} windowData - 窗户完整数据
     * @returns {Object} 框架尺寸和其他计算结果
     */
    function calculateSHPStandard(widthMm, heightMm, fixedHeightMm, frameType, glassType, windowData) {
        console.log('计算SH-P标准窗户尺寸:', { widthMm, heightMm, fixedHeightMm, frameType, glassType });
        
        // 框架尺寸计算
        const frameWidth = mmToInch(widthMm + 3 * 2);
        const frameHeight = mmToInch(heightMm + 3 * 2);
        
        // 嵌扇计算
        const sashWidth = mmToInch(widthMm - 47.4 - 2);
        const sashHeight = mmToInch((heightMm - fixedHeightMm) / 2 - 6 - 17.1 + 1);
        
        // 屏幕尺寸计算
        const screenWidth = Math.round(widthMm - 87 - 4);
        const screenHeight = Math.round((heightMm - fixedHeightMm) / 2 - 6 - 75 - 5);
        
        // 硬件部件计算
        const mullion = mmToInch(widthMm - 36);
        const mullionA = round((widthMm - 36 + 1) / 25.4 - 2, 1);
        const handleA = Math.round((widthMm - 46) / 25.4 / 2 + 4);
        const coverWidth = mmToInch(widthMm - 14 * 2 - 3 - 13);
        const coverHeight = mmToInch(fixedHeightMm - 6 - 14 * 2 - 22 * 2);
        const bigMullion = mmToInch(widthMm - 14 * 2 - 2 + 1.5);
        const slop = frameType === 'Block-slop 1/2' ? round((widthMm - 10) / 25.4, 1) : 0;
        
        // 玻璃尺寸计算
        const sashGlassWidth = widthMm - 110 - 2;
        const sashGlassHeight = (heightMm - fixedHeightMm - 6) / 2 - 79.7 - 1;
        const fixedGlassWidth = widthMm - 47;
        const fixedGlassHeight = (heightMm - fixedHeightMm - 6) / 2 - 44.2 - 1;
        const fixedGlass2Width = widthMm - 47;
        const fixedGlass2Height = fixedHeightMm - 6 - 20.5 * 2 - 3 * 2 - 1;
        
        // 格子尺寸计算
        const sashGridWidth = Math.round(sashGlassWidth - 18 - 2);
        const sashGridHeight = Math.round(sashGlassHeight - 18 - 2);
        const fixedGridWidth = Math.round(fixedGlassWidth - 18 - 2);
        const fixedGridHeight = Math.round(fixedGlassHeight - 18 - 2);
        const fixedGrid2Width = Math.round(fixedGlass2Width - 18 - 2);
        const fixedGrid2Height = Math.round(fixedGlass2Height - 18 - 2);
        
        // 根据框架类型创建框架清单
        let frameList = [];
        
        if (frameType === 'Retrofit') {
            frameList = [
                {
                    material: '82-02',
                    position: '--',
                    length: frameWidth,
                    qty: 2
                },
                {
                    material: '82-02',
                    position: '|',
                    length: frameHeight,
                    qty: 2
                }
            ];
        } else if (frameType === 'Block' || frameType === 'Block-slop 1/2') {
            frameList = [
                {
                    material: '82-01',
                    position: '--',
                    length: frameWidth,
                    qty: 2
                },
                {
                    material: '82-01',
                    position: '|',
                    length: frameHeight,
                    qty: 2
                }
            ];
        } else if (frameType === 'Block-slop') {
            frameList = [
                {
                    material: '82-02B',
                    position: '--',
                    length: frameWidth,
                    qty: 1
                },
                {
                    material: '82-01',
                    position: '--',
                    length: frameWidth,
                    qty: 1
                },
                {
                    material: '82-01',
                    position: '|',
                    length: frameHeight,
                    qty: 2
                }
            ];
        }
        
        // 嵌扇清单
        const sashList = [
            {
                material: '82-05',
                position: '--',
                length: sashWidth,
                qty: 1
            },
            {
                material: '82-05',
                position: '--',
                length: sashWidth,
                qty: 1
            },
            {
                material: '82-03',
                position: '|',
                length: sashHeight,
                qty: 2
            }
        ];
        
        // 屏幕清单
        const screenList = [
            {
                material: 'screenw',
                position: '--',
                length: screenWidth,
                qty: 2
            },
            {
                material: 'screenh',
                position: '|',
                length: screenHeight,
                qty: 2
            }
        ];
        
        // 部件清单
        const partsList = [
            {
                material: 'mullion',
                position: '--',
                length: mullion,
                qty: 1
            },
            {
                material: 'mullion aluminum',
                position: '--',
                length: mullionA,
                qty: 1
            },
            {
                material: 'handle aluminum',
                position: '--',
                length: handleA,
                qty: 1
            },
            {
                material: 'cover width',
                position: '--',
                length: coverWidth,
                qty: 1
            },
            {
                material: 'cover height',
                position: '|',
                length: coverHeight,
                qty: 1
            },
            {
                material: 'big mullion',
                position: '--',
                length: bigMullion,
                qty: 1
            }
        ];
        
        // 添加斜度部件 (如果需要)
        if (frameType === 'Block-slop 1/2') {
            partsList.push({
                material: 'slop',
                position: '--',
                length: slop,
                qty: 1
            });
        }
        
        // 计算格子清单
        const gridList = handleGridCalculations(
            windowData, 
            sashGridWidth, sashGridHeight, 
            fixedGridWidth, fixedGridHeight,
            fixedGrid2Width, fixedGrid2Height
        );
        
        // 计算玻璃清单
        const glassList = getGlassList(
            glassType, 
            sashGlassWidth, sashGlassHeight,
            fixedGlassWidth, fixedGlassHeight,
            fixedGlass2Width, fixedGlass2Height,
            windowData.top_buttom === 'Tempered'
        );
        
        // 构建结果对象
        const result = {
            frameWidth,
            frameHeight,
            frameType: frameType,
            frame: frameList,
            sash: sashList,
            screen: screenList,
            parts: partsList,
            glassList: glassList,
            gridList: gridList
        };
        
        console.log('SH-P标准窗户计算结果:', result);
        return result;
    }

    /**
     * 处理窗户数据并返回所有计算结果
     * @param {Object} windowData - 窗户基础数据
     * @returns {Object} 所有窗户计算结果
     */
    function processWindowData(windowData) {
        // 提取相关数据
        const { width, height, fixedHeight, quantity, style, customer, id, glass, argon, grid, grid_size, color, frame, top_buttom } = windowData;
        
        console.log('处理SH-P窗户数据:', {
            id, style, width, height, fixedHeight, frame, glass, grid, grid_size,
            top_buttom
        });
        
        // 转换为毫米
        const measurements = convertToMetric(width, height, fixedHeight);
        const { widthMm, heightMm, fixedHeightMm } = measurements;
        
        // 根据框架类型选择计算方法
        let calculationResults = {};
        
        if (frame === 'Nailon') {
            calculationResults = calculateSHPNailon(widthMm, heightMm, fixedHeightMm, glass, windowData);
        } else {
            calculationResults = calculateSHPStandard(widthMm, heightMm, fixedHeightMm, frame, glass, windowData);
        }
        
        console.log('SH-P窗户计算完成:', id);
        return calculationResults;
    }

    // 公开API
    return {
        processWindowData,
        convertToMetric,
        mmToInch,
        // 标准窗户计算函数
        calculateSHPStandard,
        // Nailon窗户计算函数
        calculateSHPNailon
    };
}); 