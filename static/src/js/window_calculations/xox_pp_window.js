/**
 * XOX-PP Window Style Calculations
 * Contains functions for calculating dimensions for XOX-PP style windows
 * (XOX on top, Double Picture on bottom)
 */
odoo.define('rich_custom.xox_pp_window', function (require) {
    'use strict';
     ...
     * @param {number} fixedGlass2W - 底部固定部分玻璃宽度(毫米)
     * @param {number} fixedGlass2H - 底部固定部分玻璃高度(毫米)
     * @param {boolean} isBottomTempered - 底部是否钢化
     * @returns {Array} 玻璃列表
     */
    function getGlassList(glassType, sashGlassW, sashGlassH, fixedGlassW, fixedGlassH, 
                          fixedGlass2W, fixedGlass2H, isBottomTempered) {
        let glassList = [];
     ...
     * @param {number} fixedGrid2W - 底部固定部分网格宽度
     * @param {number} fixedGrid2H - 底部固定部分网格高度
    ...
    function handleGridCalculations(windowData, sashGridW, sashGridH, fixedGridW, fixedGridH, 
        ...
        if (grid && grid !== 'None') {
            ...
            if (grid === 'Standard') {
                lightCount = gridLightsPerSection * 4; // For all four sections (2 sliding, 1 fixed, 2 bottom PP panels)
                ...
                lightCount = gridLightsPerSection * 4 * 2; // Doubled for marginal grid
                ...
                lightCount = gridLightsPerSection * 4; // For all four sections
                ...
                gridList.push({
                    ...
                    height: fixedGrid2H,
                    type: 'perimeter',
                    qty: quantity * 4 // Two PP sections, specific for perimeter
                });
            }
        }
    ...
    function calculateNailon(widthMm, heightMm, fixedHeightMm, glassType, windowData) {
        ...
        const coverWidth = (widthMm / 2) - 6 - 14 * 2 - 15 - 13;
    ...
    // 注册窗口类型处理程序
    abstractWindow.registerWindowHandler('XOX-PP', processWindowData);
