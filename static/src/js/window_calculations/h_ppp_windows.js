/**
 * H-PP Window Calculations
 * Contains functions for calculating dimensions for H-PP style windows
 */
odoo.define('rich_production.h_ppp_windows', [], function (require) {
    'use strict';
    ...
    function handleGridCalculations(windowData, fixedgridw, fixedgridh) {
        ...
        console.log('H-PPP handleGridCalculations被调用，参数:', {
        ...
        console.log('H-PPP handleGridCalculations返回结果:', gridList);
        return gridList;
    }

    /**
     * 计算H-PP窗户的Nailon型材框架尺寸
     ...
     */
    function calculateHPPNailon(widthMm, heightMm, glassType, windowData) {
        console.log('计算H-PPP Nailon窗户尺寸:', { widthMm, heightMm, glassType });
        ...
        // 部件计算
        const coverWidth = mmToInch(widthMm / 2 - 6 - 14 * 2 - 15 - 3 - 13);
        const coverHeight = mmToInch(heightMm - 14 * 2 - 15 * 2 - 22 * 2);
        const bigMullion = mmToInch(heightMm - 14 * 2 - 15 * 2 - 2 + 1.5);
        
        // 玻璃尺寸计算
        const fixedGlassWidth = widthMm / 3 - 6 - 20.5 * 2 - 3 * 2 - 15 - 3;
        const fixedGlassHeight = heightMm - 20.5 * 2 - 3 * 2 - 15 * 2 - 3;
        const middlefixedGlassWidth = widthMm / 3 - 6 - 20.5 * 2 - 3 * 2 - 3;
        const middlefixedGlassHeight = heightMm - 20.5 * 2 - 3 * 2 - 15 * 2 - 3;
        ...
        console.log('H-PPP Nailon窗户计算结果:', result);
        return result;
    }

    /**
     * 计算H-PP窗户的标准框架尺寸 (Retrofit/Block/Block-slope/Block-slope 1/2)
     * @param {number} widthMm - 窗户宽度(毫米)
     * @param {number} heightMm - 窗户高度(毫米)
     ...
     * @returns {Object} 框架尺寸和其他计算结果
     */
    function calculateHPPStandard(widthMm, heightMm, frameType, glassType, windowData) {
        console.log('计算H-PP标准窗户尺寸:', { widthMm, heightMm, frameType, glassType });
        ...
        // 部件计算
        const coverWidth = mmToInch(widthMm / 2 - 6 - 14 * 2 - 3 - 13);
        const coverHeight = mmToInch(heightMm - 14 * 2 - 22 * 2);
        const bigMullion = mmToInch(heightMm - 14 * 2 - 2 + 1.5);
        const slop = frameType === 'Block-slop 1/2' ? round((widthMm - 10) / 25.4, 1) : 0;
        
        // 玻璃尺寸计算
        const fixedGlassWidth = widthMm / 3 - 6 - 20.5 * 2 - 3 * 2 - 3;
        const fixedGlassHeight = heightMm - 20.5 * 2 - 3 * 2 - 3;
        const middlefixedGlassWidth = widthMm / 3 - 6 - 20.5 * 2 - 3 * 2 - 3;
        const middlefixedGlassHeight = heightMm - 20.5 * 2 - 3 * 2  - 3;
        ...
        console.log('H-PP标准窗户计算结果:', result);
        return result;
    }
    ...
    function processWindowData(windowData) {
        ...
        if (frame === 'Nailon') {
            calculationResults = calculateHPPNailon(widthMm, heightMm, glass, windowData);
        } else {
            calculationResults = calculateHPPStandard(widthMm, heightMm, frame, glass, windowData);
        }
        
        console.log('H-PP窗户计算完成:', id);
