/**
 * PPP-XOX Window Style Calculations
 * Contains functions for calculating dimensions for PPP-XOX style windows
 * (Triple Picture on bottom, XOX on top)
 */
odoo.define('rich_custom.ppp_xox_window', function (require) {
    'use strict';
     ...
     * @param {number} fixedGlass2W - 底部左右侧固定部分玻璃宽度(毫米)
     * @param {number} fixedGlass2H - 底部左右侧固定部分玻璃高度(毫米)
     * @param {number} fixedGlass3W - 底部中间固定部分玻璃宽度(毫米)
     * @param {number} fixedGlass3H - 底部中间固定部分玻璃高度(毫米)
     * @param {boolean} isBottomTempered - 底部是否钢化
     * @returns {Array} 玻璃列表
     */
    function getGlassList(glassType, sashGlassW, sashGlassH, fixedGlassW, fixedGlassH, 
                          fixedGlass2W, fixedGlass2H, fixedGlass3W, fixedGlass3H, isBottomTempered) {
        let glassList = [];
        const isTempered = glassType.includes('Tmp');
        const tmprd = isTempered ? 'T' : '';
        const bottomTmprd = isTempered || isBottomTempered ? 'T' : '';
        ...
        // Sliding Sections (X) at top - There are two sliding sections (left and right) - Line 1
        glassList.push({
            line: 1,
        ...
        // Fixed Section (O) in the middle at top - Line 2
        glassList.push({
            line: 2,
        ...
        // Bottom Left/Right PP Sections - Line 3 (two equal panels on sides)
        glassList.push({
            line: 3,
        ...
        // Bottom Middle PP Section - Line 4 (middle panel)
        glassList.push({
            line: 4,
     ...
     * @param {number} fixedGrid2W - 底部左右侧固定部分网格宽度
     * @param {number} fixedGrid2H - 底部左右侧固定部分网格高度
     * @param {number} fixedGrid3W - 底部中间固定部分网格宽度
     * @param {number} fixedGrid3H - 底部中间固定部分网格高度
    ...
    function handleGridCalculations(windowData, sashGridW, sashGridH, fixedGridW, fixedGridH, 
        ...
        if (grid && grid !== 'None') {
            ...
            if (grid === 'Standard') {
                lightCount = gridLightsPerSection * 5; // For all five sections (2 sliding, 1 fixed, 2 bottom PP panels)
                configTop = baseConfig;
                configMiddle = baseConfig;
                configBottom = baseConfig;
                
                // Add grids to the list
                // Sliding section grids (left and right) at top
                gridList.push({
                    section: 'sliding',
                    position: 'left-right',
                    width: sashGridW,
                ...
                // Bottom left/right PP sections
                gridList.push({
                    section: 'bottom-sides',
                    width: fixedGrid2W,
                    height: fixedGrid2H,
                    type: 'standard',
     ...
     * @param {number} fixedHeightMm - 底部固定部分高度(毫米)
     * @param {string} glassType - 玻璃类型
     * @param {Object} windowData - 窗户数据
    ...
    function calculateNailon(widthMm, heightMm, fixedHeightMm, glassType, windowData) {
        ...
        // XOX section height (top part)
        const xoxSectionHeight = heightMm - fixedHeightMm;
        
        // Sash dimensions for sliding parts (X) - left and right at top
        ...
        // Glass dimensions
        // For sliding parts (X) - left and right at top
        const sashGlassW = widthMm / 4 - 77 - 15;
        const sashGlassH = xoxSectionHeight - 6 - 109 - 15 - 3 - 2;
        
        // For fixed part (O) in the middle at top
        const fixedGlassW = widthMm / 2 - 41.4;
        const fixedGlassH = xoxSectionHeight - 6 - 47 - 15 - 2;
        
        // For bottom PP parts - left and right
        const fixedGlass2W = widthMm / 4 - 6 - 20.5 * 2 - 3 * 2 - 15;
        const fixedGlass2H = fixedHeightMm - 6 - 20.5 * 2 - 3 * 2 - 15 - 2;
        
        // For bottom PP part - middle
        ...
        // Check if bottom is tempered
        const isBottomTempered = windowData.topBottom === 'Tempered';
    ...
    function calculateOtherFrames(widthMm, heightMm, fixedHeightMm, frame, glassType, windowData) {
        ...
        const sashGlassH = xoxSectionHeight - 6 - 109 - 3 - 2;
        
        // For fixed part (O) in the middle at top
        const fixedGlassW = widthMm / 2 - 41.4;
        const fixedGlassH = xoxSectionHeight - 6 - 47 - 2;
    ...
    // 注册窗口类型处理程序
    abstractWindow.registerWindowHandler('PPP-XOX', processWindowData);
