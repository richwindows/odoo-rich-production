/**
 * PP-XOX Window Style Calculations
 * Contains functions for calculating dimensions for PP-XOX style windows
 * (Double Picture on top, XOX on bottom)
 */
odoo.define('rich_custom.pp_xox_window', function (require) {
    'use strict';
    ...
    function getGlassList(glassType, sashGlassW, sashGlassH, fixedGlassW, fixedGlassH, 
                          fixedGlass2W, fixedGlass2H, isTopTempered) {
        let glassList = [];
        ...
        // Sliding Sections (X) at bottom - There are two sliding sections (left and right) - Line 2
        glassList.push({
            line: 2,
        ...
        // Fixed Section (O) in the middle at bottom - Line 3
        glassList.push({
            line: 3,
    ...
    function handleGridCalculations(windowData, sashGridW, sashGridH, fixedGridW, fixedGridH, 
        ...
        if (grid && grid !== 'None') {
            ...
            if (grid === 'Standard') {
                lightCount = gridLightsPerSection * 4; // For all four sections (2 top PP panels, 2 sliding, 1 fixed)
                ...
                // Sliding section grids (left and right) at bottom
                gridList.push({
                    section: 'sliding',
                    position: 'left-right',
                    width: sashGridW,
                ...
                // Middle fixed section at bottom
                gridList.push({
                    section: 'fixed',
                    position: 'middle',
                    width: fixedGridW,
    ...
    function calculateNailon(widthMm, heightMm, fixedHeightMm, glassType, windowData) {
        ...
        // Sash dimensions for sliding parts (X) - left and right at bottom
        ...
        // Glass dimensions
        // For sliding parts (X) - left and right at bottom
        const sashGlassW = widthMm / 4 - 77 - 15;
        const sashGlassH = xoxSectionHeight - 6 - 109 - 15 - 3 - 2;
        
        // For fixed part (O) in the middle at bottom
    ...
    // 注册窗口类型处理程序
    abstractWindow.registerWindowHandler('PP-XOX', processWindowData);
